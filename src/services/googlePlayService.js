...
export const isPlayBillingAvailable = async () => {
...
export const purchaseSubscription = async (userId) => {
    try {
        if (!('getDigitalGoodsService' in window))
            throw new Error("Google Play indisponível");
        const service = await window.getDigitalGoodsService('https://play.google.com/billing');
...
const acknowledgePurchase = async (userId, token) => {
...
export const checkActivePurchases = async (userId) => {
    try {
        if (!('getDigitalGoodsService' in window))
            return;
        const service = await window.getDigitalGoodsService('https://play.google.com/billing');
